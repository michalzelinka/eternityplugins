  ...
  case WM_COMMAND:
      ...
      break;
		...
		case IDC_IGNORECHECK:
				...
				
			{
				...
				SetDlgItemText(hwndDlg, IDC_IGNORECHECK, Translate("Checking..."));
	...
	case HM_PROTOACK:
		{
			...
			{
				SetDlgItemText(hwndDlg, IDC_IGNORECHECK, Translate("Msg Accepted!"));
				Netlib_Logf(ghServerNetlibUser, "User %u accepts your messages", dat->dwUin);
			}
			else
			if (ack->result == ACKRESULT_FAILED)
			{
				SetDlgItemText(hwndDlg, IDC_IGNORECHECK, Translate("Msg Ignored!"));
	...
	case WM_TIMER:
		...
		if (wParam==TIMEOUT_IGNORE)
		{
			...
			SetDlgItemText(hwndDlg, IDC_IGNORECHECK, Translate("Ignore Check"));//set back
		}
		else
