      ...
      default:
        {
          ...
          handleMessageTypes(dwUin, time(NULL), dwID1, dwID2, wCookie, wVersion, bMsgType, bFlags, wAckType, tlv->wLen - 53, wMsgLen, pDataBuf, 0, &pMsgAck);
          break;
        }
      }
    }
...
void parseServerGreeting(BYTE* pDataBuf, WORD wLen, WORD wMsgLen, DWORD dwUin, BYTE bFlags, WORD wStatus, WORD wCookie, WORD wAckType, DWORD dwID1, DWORD dwID2, WORD wVersion)
{
  ...
  //    handleChatRequest(pDataBuf, wLen, dwUin, wCookie, dwID1, dwID2, szMsg, 8);
    ...
    else if (typeId)
    {
      ...
      pMsgAck.bFlags = bFlags;
      handleMessageTypes(dwUin, time(NULL), dwID1, dwID2, wCookie, wVersion, typeId, bFlags, wAckType, dwLengthToEnd, (WORD)dwDataLen, pDataBuf, MTF_PLUGIN, &pMsgAck);
    }
    else
    {
      ...
      else
      {
        ...
        if (bMsgType == MTYPE_PLUGIN)
        {
          ...
          if (unpackPluginTypeId(&pmsg, &wLen, &typeId, NULL, FALSE) && wLen > 8)
          {
            ...
            if (typeId)
              handleMessageTypes(dwUin, dwRecvTime, dwMsgID1, dwMsgID2, 0, 0, typeId, bFlags, 0, dwLengthToEnd, (WORD)dwDataLen, pmsg, MTF_PLUGIN, NULL);
            else
            {
              NetLog_Server("Unsupported plugin message type %d", typeId);
            }
          ...
          handleMessageTypes(dwUin, dwRecvTime, dwMsgID1, dwMsgID2, 0, 0, bMsgType, bFlags, 0, wTLVLen - 8, wMsgLen, pmsg, 0, NULL);
      }
    }
    else
    {
...
BOOL handleMessageTypes(DWORD dwUin, DWORD dwTimestamp, DWORD dwMsgID, DWORD dwMsgID2, WORD wCookie, WORD wVersion, int type, int flags, WORD wAckType, DWORD dwDataLen, WORD wMsgLen, char *pMsg, int nMsgFlags, message_ack_params *pAckParams)
{
  ...
  if (wAckType == 2)
  {
	  ...
	  chk.dwUin=dwUin;
	  CheckContact(chk);
    handleMessageAck(dwUin, wCookie, wVersion, type, wMsgLen, pMsg, (BYTE)flags, nMsgFlags);
	res = FALSE;
    return res;
  }
