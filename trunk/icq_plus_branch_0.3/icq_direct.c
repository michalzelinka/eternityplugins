...
// Last change on : $Date: 2007-08-22 04:03:43 +0300 (Ð¡Ñ€, 22 Ð°Ð²Ð³ 2007) $
...
void InitDCEvents(void)
{
  ...
  IconLibDefine(ICQTranslateUtfStatic("Direct Connection", str, MAX_PATH), szSection, "dcico", LoadIcon(hIconInst, MAKEINTRESOURCE(IDI_DC)), NULL, -(IDI_DC),0);
...
static void setContactExtraIcon(HANDLE hContact, BOOL bFunc, BOOL bRem)
{
	...
	else
	{
		...
		ico.hExtraIcon=hExtraDC;
		ico.icoName=((IsDCOpen(hContact) || bFunc) && DBGetContactSettingByte(NULL, "ICQ", "ShowDCIcon", 1) && !bRem ? "dcico" : (char*)0);
