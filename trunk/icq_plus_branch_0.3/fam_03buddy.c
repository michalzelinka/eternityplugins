...
{
  BYTE i;
  extern GetQipStatusIcon(int bStatus, UINT flags);
...
void InitQipStatusEvents()
{
  if (!hHookQStatusBuild)
      ...
      hHookQStatusBuild = HookEvent(ME_CLIST_PREBUILDSTATUSMENU, CListMW_BuildStatusItems);

  if (!hHookQipIconsRebuild)
    hHookQipIconsRebuild = HookEvent(ME_CLIST_EXTRA_LIST_REBUILD, CListMW_ExtraIconsRebuild);

  if (!hHookQipIconsApply)
    hHookQipIconsApply = HookEvent(ME_CLIST_EXTRA_IMAGE_APPLY, CListMW_ExtraIconsApply);
}
...
static void setContactQipExtraIcon(HANDLE hContact, int qipstatus)
{
	...
	else
	{
		...
		else
		{
			...
			ico.cbSize=sizeof(ico);
			null_snprintf(szTemp, sizeof(szTemp), "%s_QIPstatus%d", gpszICQProtoName, qipstatus-1);
			ico.icoName=szTemp;
			ico.hContact=hContact;
			ico.hExtraIcon=hExtraqipstatus;
			CallService(MS_EXTRAICON_SET_ICON, (WPARAM)&ico, 0);
