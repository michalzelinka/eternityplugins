...
static char* InitXStatusIconLibrary(char* buf)
{
  ...
  if( hIconFolder ) {
	...
	if (!hIconsDLL)
		hIconsDLL = LoadXStatusIconLibrary(path, "");
  }
  if (!hIconsDLL) {
  ...
  if (!hIconsDLL)
     hIconsDLL = LoadXStatusIconLibrary(path, "\\Icons");

  if (!hIconsDLL)
     hIconsDLL = LoadXStatusIconLibrary(path, "\\Plugins");
...
static void setContactExtraIcon(HANDLE hContact, int xstatus)
{
	...
	else
	{
		...
		else
		{
			...
			ico.cbSize=sizeof(ico);
			null_snprintf(szTemp, sizeof(szTemp), "%s_xstatus%d", gpszICQProtoName, xstatus-1);
			ico.icoName=szTemp;
			ico.hContact=hContact;
			ico.hExtraIcon=hExtraXStatus;
			CallService(MS_EXTRAICON_SET_ICON, (WPARAM)&ico, 0);
...
void InitXStatusItems(BOOL bAllowStatus)
{
  ...
  while  (i < XstatusItems)
  {
    ...
    sprintf(szTemp, "XStatus%dName", j); szValue = ICQGetContactSettingUtf(NULL, szTemp, ""); utf8_decode(szValue, &ansi);
...
void InitXStatusIcons()
{
  ...
  null_snprintf(szSection, sizeof(szSection), ICQTranslateUtfStatic("Status Icons/%s/Custom Status", str, MAX_PATH), ICQTranslateUtfStatic(gpszICQProtoName, prt, MAX_PATH));
