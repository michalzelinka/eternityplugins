...
struct CIcqProto : public PROTO_INTERFACE
{
	...
	HANDLE m_hContactMenuItems[10]; // 5 + 5
  ...
  void   handleServerCListRightsReply(BYTE *buf, WORD wLen);
	...
	//////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////
	//  ICQ PLUS MOD IMPLEMENTATION                                         //
	//////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////

	// menu item handles + icons
	//HANDLE hIconMenuSendTzer, hIconMenuAlwaysVisible, hIconMenuNeverVisible, \
	       hIconMenuIncognito, hIconMenuScan, hIconMenuScanStart, hIconMenuScanStop, \
	       hIconMenuScanPause, hIconMenuHidden, hIconMenuManageServList, \
	       hIconMenuAdvFeatures, hIconMenuDisableItem, hIconMenuEnableItem;

	HANDLE /*hUserMenuSendTzer, hUserMenuAlwaysVisible, hUserMenuNeverVisible, \
	       hUserMenuIncognito, hUserMenuScan,*/ hUserMenuScanStart, hUserMenuScanStop, \
	       hUserMenuScanPause, hUserMenuHidden, hUserMenuManageServList, \
	       hUserMenuAdvFeatures, hUserMenuDisableItem, hUserMenuEnableItem;

	// advanced features
	BOOL   m_bWebAware;
	BOOL   m_bSecureIM;
	HANDLE m_hMenuAdvFeaturesRoot;
	HANDLE m_hMenuUsersStatusScan;
	HANDLE hAdvFeaturesItems[11];

	void   InitAdvancedFeaturesItems();
	int    __cdecl menuAdvFeatures(WPARAM wParam, LPARAM lParam, LPARAM fParam);
	void   SendAuthRequestToAllUnauthorized() {};

	// client id + capabilities
	BOOL   m_bHideIdEnabled;
	BOOL   m_bCustomProtoVersionEnabled;
	BOOL   m_bCustomCapsEnabled;
	BOOL   m_bRTFEnabled;
	HWND   m_hCapsList;
	HWND   m_hCapsListCaller;
	WORD   m_wCapsCount;
	WORD   m_wClass;
	...
	// incognito
	BOOL   m_bIncognitoRequest;
	BOOL   m_bIncognitoGlobal;

	int    __cdecl IncognitoAwayRequest(WPARAM wParam, LPARAM lParam);
	DWORD  icq_sendGetStealthAwayMsgServ(HANDLE hContact, DWORD dwUin, int type, WORD wVersion);

	// invisible detection
	BOOL   m_bASD;
	BOOL   m_bPSD;
	BOOL   m_bNoASDInInvisible;
	BOOL   m_bNoPSDForHidden;
	BOOL   m_bASDForOffline;
	BOOL   m_bASDViaAwayMsg;
	BOOL   m_bASDViaXtraz;
	BOOL   m_bASDViaURL;
	BOOL   m_bASDUnauthorized;
	BOOL   m_bASDViaAuth;
	HWND   m_hASDDialogCaller;
	WORD   m_wCheckSpeed;
	WORD   m_wCheckSpeedDefault;
	BYTE   m_FLCStatus;

	void   CheckContact(CHECKCONTACT chk);
	void   icq_CheckSpeed(WORD wStatus);
	void   setAdvFeaturesIcons(int o);
	void   icq_CheckAllUsersStatus(int a, int b) {};

	// logging
	BOOL   m_bLogSelfRemoveFile;
	BOOL   m_bLogIgnoreCheckFile;
	BOOL   m_bLogStatusCheckFile;
	BOOL   m_bLogClientChangeFile;
	BOOL   m_bLogAuthFile;
	BOOL   m_bLogInfoRequestFile;
	BOOL   m_bLogASDFile;
	BOOL   m_bLogSelfRemoveHistory;
	BOOL   m_bLogIgnoreCheckHistory;
	BOOL   m_bLogStatusCheckHistory;
	BOOL   m_bLogClientChangeHistory;
	BOOL   m_bLogAuthHistory;
	BOOL   m_bLogInfoRequestHistory;
	BOOL   m_bLogASDHistory;
	BOOL   m_bLogToContactsHistory;

	//void   ProcessEvent(); // merged function for calling both logging to file+history and popuping
	...
	// multi server
	int    m_iIcqNewStatus;
	BOOL   m_bServerAutoChange;
	HWND   m_hSrvsList;
	HWND   m_hSrvsListCaller;
	BOOL   m_bTryNextWorking;
	...
	// popups
	HWND   hPopupDialog;
	HWND   hPopupMoreDialog;

	// privacy
	BOOL   m_bNoStatusReply;
	BYTE   m_bVisibility;

	BOOL   InvisibleForContact(DWORD dwUin, HANDLE hContact);
	...
	// tzers
	BOOL   m_bTzersEnabled;

	int    __cdecl SendTzerService(WPARAM wParam, LPARAM lParam);
	int    __cdecl SendTzerDialog(WPARAM wParam, LPARAM lParam);
	void   SendTzer(TZERTOSEND* tts);

	// utilities
	int    GetFromContactsCacheByID(int ID, HANDLE *hContact, DWORD *dwUin);
	void   NickFromHandleStatic(HANDLE hContact, char *szNick, WORD wLen);
	void   makeUserOffline(HANDLE hContact);
	int    CacheIDCount();
	int    IDFromContactsCacheByUin(DWORD dwUin);
	int    SetContactsCacheFlagsByID(int ID, DWORD flags);
	int    GetContactsCacheByID(int ID, icq_contacts_cache *icc);
};
