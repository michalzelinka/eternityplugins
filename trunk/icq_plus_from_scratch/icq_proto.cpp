...
CIcqProto::~CIcqProto()
{
	...
	//SAFE_FREE(&m_modeMsgs.szOnline); // maybe, too?
  ...
  if (!icqOnline())
	...
	if (dwUin)
	{
		...
		if (wMessageType)
		{
			...
			{
				m_bIncognitoRequest = FALSE;
				return icq_sendGetStealthAwayMsgServ(hContact, dwUin, wMessageType,
    ...
    { // update current status note
      ...
      if (getSettingByte(NULL, "XStatusMsgAsNote", DEFAULT_XSTATUS_STATUS_NOTE))
      {
        ...
        SetStatusNote(szNote, 1000, FALSE);
  		...
  		if (m_bAimEnabled && (m_iStatus == status) && !m_bNoStatusReply)
