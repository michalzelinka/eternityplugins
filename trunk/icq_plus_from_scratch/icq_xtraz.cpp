...
// -----------------------------------------------------------------------------
//
// File name      : $URL: https://miranda.svn.sourceforge.net/svnroot/miranda/trunk/miranda/protocols/IcqOscarJ/icq_xtraz.cpp $
					...
					if ((DWORD)atoi(szSender) == dwUin)
					{
						...
						{ // apply privacy rules
							...
							if (!m_bXStatusEnabled) {
								...
								chk.dwUin = dwUin;
								chk.hContact = hContact;
								chk.bLogToFile = chk.bShowPopup = chk.bHistoryEvent = TRUE;
								chk.iICQEventType = ICQEVENTTYPE_CHECKSTATUS;
								chk.pszMsg = "Xtraz notify when feature is disabled";
...
DWORD CIcqProto::SendXtrazNotifyRequest(HANDLE hContact, char* szQuery, char* szNotify, int bForced)
{
	...
	if (InvisibleForContact(dwUin, hContact))
		return 0; // We don't want to be detected by the other side
