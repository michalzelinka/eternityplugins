...
$Id: msgs.c 6525 2007-10-04 12:43:49Z nightwish2004 $
...
static int SplitmsgModulesLoaded(WPARAM wParam, LPARAM lParam)
{
	...
	BOOL bIEView;
	static Update upd = {0};
// eternity :: updater paths - part 1
#if defined(_UNICODE)
	static char szCurrentVersion[30];
	static char *szVersionUrl = "http://dev.mirandaim.ru/jarvis/";
	static char *szUpdateUrl = "http://mirandapack.ic.cz/eternity_plugins/tabsrmm.zip";
...
//	static char *szFLUpdateurl = "http://addons.miranda-im.org/feed.php?dlfile=1401";
#endif    
	static char *szPrefix = "tabSRMM eternity Mod</a> ";
	...
	*/
	...
	// updater plugin support :: eternity version - part 2
...
#if defined(_UNICODE)
	upd.pbVersionPrefix = (BYTE *)"tabSRMM eternity Mod</a> ";
//#else
//	upd.pbVersionPrefix = (BYTE *)"<span class=\"fileNameHeader\">tabSRMM Mod (ANSI VC8) ";
#endif
...
// eternity: release notes dialog
  {
    int relnotes;
    relnotes = DBGetContactSettingWord( NULL, SRMSGMOD_T, "MOD_RelNotes", 0 );
    if ( relnotes != TABSRMM_RELNOTES )
    {
      extern INT_PTR CALLBACK RelNotesProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
      DialogBox( g_hInst, MAKEINTRESOURCE( IDD_RELNOTES ), NULL, ( DLGPROC )RelNotesProc );
      DBWriteContactSettingWord( NULL, SRMSGMOD_T, "MOD_RelNotes", TABSRMM_RELNOTES );
    }
  }
// eternity: status button for turning typing sounds on/off
	if ( ServiceExists( MS_MSG_ADDICON ) ) {
    int isOn;
		StatusIconData sid = { 0 };
		sid.cbSize = sizeof( sid );
		sid.szModule = SRMSGMOD_T;
    sid.dwId = 88801;
    sid.hIcon = (HANDLE)CallService(MS_SKIN2_GETICON, 0, (LPARAM)"tabSRMM_mod_typing_sounds_on");
    sid.hIconDisabled = (HANDLE)CallService(MS_SKIN2_GETICON, 0, (LPARAM)"tabSRMM_mod_typing_sounds_off");

    isOn = DBGetContactSettingByte( NULL, SRMSGMOD_T, SRMSGSET_MOD_TYPINGSOUND, SRMSGDEFSET_MOD_TYPINGSOUND );
    if ( isOn == 1 )
    {
	    sid.flags = 0;
	    sid.szTooltip = "Typing Sounds On";
    }
    else
    {
	    sid.flags = MBF_DISABLED;
	    sid.szTooltip = "Typing Sounds Off";
    }
		CallService(MS_MSG_ADDICON, 0, (LPARAM) &sid);
		HookEvent( ME_MSG_ICONPRESSED, TSoundsProcessSrmmIconClick );
	}
	return 0;
}
...
// eternity mod #24
  	SkinAddNewSoundEx("TypingBell", Translate("Messages"), Translate("Typing Bell"));
  	SkinAddNewSoundEx("TypingBack", Translate("Messages"), Translate("Typing Backspace"));
  	SkinAddNewSoundEx("TypingType", Translate("Messages"), Translate("Typing Typing"));
// eternity mod #24 (end)
...
static ICONDESC _deficons[] = {
    ...
    "tabSRMM_mod_typing_sounds_on", LPGEN("Typing Sounds On"), &myGlobals.g_buttonBarIcons[29], -IDI_TSOUNDSON, 1,
    "tabSRMM_mod_typing_sounds_off", LPGEN("Typing Sounds Off"), &myGlobals.g_buttonBarIcons[30], -IDI_TSOUNDSOFF, 1,
    NULL, NULL, NULL, 0, 0
};
...
// eternity: clicking on Status Bar Typing Sounds button
int TSoundsProcessSrmmIconClick( WPARAM wParam, LPARAM lParam )
{
  ...
  if ( sicd->dwId == 88801 )
  {
    int isOn;
	  StatusIconData sid = { 0 };
    isOn = DBGetContactSettingByte( NULL, SRMSGMOD_T, SRMSGSET_MOD_TYPINGSOUND, SRMSGDEFSET_MOD_TYPINGSOUND );
    if ( isOn == 1 ) DBWriteContactSettingByte( NULL, SRMSGMOD_T, SRMSGSET_MOD_TYPINGSOUND, 0 );
    else DBWriteContactSettingByte( NULL, SRMSGMOD_T, SRMSGSET_MOD_TYPINGSOUND, 1 );

	  sid.cbSize = sizeof( sid );
	  sid.szModule = SRMSGMOD_T; // was "" x)
    sid.dwId = 88801;
    if ( isOn == 1 )
    {
	    sid.flags = MBF_DISABLED;
