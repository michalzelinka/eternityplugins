...
// eternity: new xstatus icons + corrected old names x)
TCHAR *xStatusDescr[] = {	_T("Angry"), _T("Taking a bath"), _T("Tired"), _T("Drinking beer"), _T("Beer"), _T("Thinking"), _T("Eating"), 
								_T("Watching TV"), _T("Meeting"), _T("Coffee"),	_T("Listening to music"), _T("Business"), _T("Shooting"), _T("Having fun"), 
								_T("On the phone"), _T("Gaming"), _T("Studying"), _T("Shopping"), _T("Feeling sick"), _T("Sleeping"),
								_T("Surfing"), _T("Browsing"), _T("Working"), _T("Typing"), _T("Picnic"), 
								_T("Cooking"), _T("Smoking"),	_T("I am High"), _T("On WC"), _T("To be or not to be"), 
								_T("Watching pro7 on TV"), _T("Love"), _T("Searching"), _T("Love"), _T("Journal"), _T("Sex"), _T("Smoking")
							};
...
// eternity mod #24
			/* TODO:
			 * repair the Typing Bell (plays sound everytime Enter key is pressed,
			 * should be differend for sendOnEnter - sendOnDblEnter - sendOnShftEnter)
			 * MAYBE solved on lines 89x - .. (switching wparam VK_RETURN)                  				 
			 */
				if (DBGetContactSettingByte(NULL, SRMSGMOD_T, SRMSGSET_MOD_TYPINGSOUND, SRMSGDEFSET_MOD_TYPINGSOUND)) {
			/* OLD SOLUTION - before 89x
          if (wParam=='\n' || wParam=='\r') SkinPlaySound("TypingBell");
			    else if (wParam!='\b') SkinPlaySound("TypingType");
					else SkinPlaySound("TypingBack"); 
          */
          /* NEW SOLUTION */
          if (wParam=='\b') SkinPlaySound("TypingBack");
          else if (((wParam!='\n' || wParam!='\r') && wParam!=VK_RETURN)) SkinPlaySound("TypingType");
				}
// eternity mod #24 (end)
		...
		case WM_KEYDOWN: {
			...
			if (wParam == VK_RETURN) {
				if (isShift) {
					...
					/* break; OLD SOLUTION */
          { SkinPlaySound("TypingBell"); break; }
          /* eternity mod #24 NEW SOLUTION */
				}
...
// error control
		...
		case DM_ACTIVATETOOLTIP: {
			...
			if (dat->hwndTip) {
				...
				switch (id) {
					case IDC_PANELNICK: {
						...
						if (dat->xStatus > 0 && dat->xStatus <= 38) { // eternity: new xstatus icons
					...
					case IDC_PANELSTATUS + 1: {
					  // eternity TODO : tipper instead of windows tooltip
            ...
            else if (dat->xStatus > 0 && dat->xStatus <= 38) // eternity: new xstatus icons
