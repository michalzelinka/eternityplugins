...
// -----------------------------------------------------------------------------
//
// File name      : $URL: https://miranda.svn.sourceforge.net/svnroot/miranda/trunk/miranda/protocols/IcqOscarJ/fam_04message.cpp $
...
//	case MTYPE_AUTOONLINE: // TODO: Could this be useful?
	case MTYPE_AUTOAWAY:
	case MTYPE_AUTOBUSY:
	case MTYPE_AUTONA:
	case MTYPE_AUTODND:
...
void packPluginTypeId(icq_packet *packet, int nTypeID)
{
	...
	case MTYPE_TZER_MESSAGE:
		packLEWord(packet, 0x32);              // Length

		packGUID(packet, MGTYPE_TZER_MESSAGE); // Message Type GUID
		packWord(packet, 0x0000);              // Unknown
		packLEDWord(packet, 0x000D);           // Unknown
		packBuffer(packet, (LPBYTE)"T-Zer Message", 0x000D);

		packDWord(packet, 0x00000000);         // Unknown binary stuff
		packDWord(packet, 0x00000000);
		packDWord(packet, 0x00000000);
		packWord(packet, 0x0000);
		packByte(packet, 0x00);
...
void CIcqProto::handleMessageTypes(DWORD dwUin, DWORD dwTimestamp, DWORD dwMsgID, DWORD dwMsgID2, WORD wCookie, WORD wVersion, int type, int flags, WORD wAckType, DWORD dwDataLen, WORD wMsgLen, char *pMsg, int nMsgFlags, message_ack_params *pAckParams)
{
	...
	case MTYPE_TZER_MESSAGE:    /* tZer message */
		if (m_bTzersEnabled)
		{
			...
			char *pszMessage = NULL, *pszUrl = NULL, *pszOutput = NULL;

			// Start of tZer "packet" processing -- TODO: I think it would be really
			// fine to re-implement this using core XML parser instead of this risky
			// code x))
			for (int i = 0; i < wMsgLen; i++)
			{
				// getting message
				if (bMessageRead == TRUE)
				{
					if (szMsg[i] == '"')
					{
						if (pszMessage)
						{
							pszMessage = (char *)realloc(pszMessage, dwMessageLen + 1);
							pszMessage[dwMessageLen] = 0;
						}
						bMessageRead = FALSE;
					}
					else
					{
						dwMessageLen++;
						if (dwMessageLen == 1)
							pszMessage = (char *)malloc(dwMessageLen);
						else
							pszMessage = (char *)realloc(pszMessage, dwMessageLen );

						pszMessage[dwMessageLen - 1] = szMsg[i];
					}
				}
				...
				if (bUrlRead == TRUE)
				{
					if (szMsg[i] == '"')
					{
						if (pszUrl)
						{
							pszUrl = (char *)realloc(pszUrl, dwUrlLen + 1);
							pszUrl[dwUrlLen] = 0;
						}
						bUrlRead = FALSE;
					}
					else
					{
						dwUrlLen++;
						if (dwUrlLen == 1)
							pszUrl = (char *)malloc(dwUrlLen);
						else
							pszUrl = (char *)realloc(pszUrl, dwUrlLen );

						pszUrl[dwUrlLen - 1] = szMsg[i];
					}
				}
			...
			pszMessage = DemangleXml(pszMessage, dwMessageLen);
			pszUrl = DemangleXml(pszUrl, dwUrlLen);
			pszOutput = (char *)malloc(22 + dwMessageLen + dwUrlLen + 1);
			sprintf(pszOutput, "You have been tZed: %s\r\n%s", pszMessage, pszUrl);
			...
			pre.timestamp = dwTimestamp;
			pre.szMessage = pszOutput;
			pre.lParam = 0;

			CallService(MS_PROTO_CHAINRECV, 0, (LPARAM)&ccs);
			SAFE_FREE((void**)&pszMessage);
			SAFE_FREE((void**)&pszUrl);
			SAFE_FREE((void**)&pszOutput);
		}
		break;
