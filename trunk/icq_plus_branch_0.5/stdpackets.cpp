...
// -----------------------------------------------------------------------------
//
// File name      : $URL: https://miranda.svn.sourceforge.net/svnroot/miranda/trunk/miranda/protocols/IcqOscarJ/stdpackets.cpp $
...
DWORD CIcqProto::icq_SendChannel2Message(DWORD dwUin, HANDLE hContact, const char *szMessage, int nBodyLen, WORD wPriority, cookie_message_data *pCookieData, char *szCap)
{
	...
	// Pack the standard header
	packServChannel2Header(&packet, this, dwUin, (WORD)(nBodyLen + (szCap ? 53:11)), pCookieData->dwMsgID1, pCookieData->dwMsgID2, dwCookie, GetProtoVersion(), (BYTE)pCookieData->bMessageType, 0,
...
void CIcqProto::SendTzer(TZERTOSEND* tts)
{
	icq_packet packet;
	WORD wCoreLen;
	DWORD dwCookie;
	...
	DWORD dwUin = getContactUin(tts->hContact);

	char * szBody = (char *)malloc((66 + 2 + strlen(tts->pszText) + strlen(tts->pszURL) + getUINLen(dwUin) + 1) * sizeof(char));
	sprintf(szBody, "<tzer id=\"ID\" name=\"%s\" url=\"%s\" preview=\"0\" freeData=\"preview=0;uin=%lu\"/>", tts->pszText, tts->pszURL, dwUin);
	nBodyLen = strlen(szBody);

	pCookieData = CreateMessageCookie(MTYPE_TZER_MESSAGE, ACKTYPE_CLIENT);
	dwCookie = AllocateCookie(CKT_MESSAGE, 0, tts->hContact, (void*)pCookieData);
	wCoreLen = 11 + getPluginTypeIdLen(pCookieData->bMessageType) + nBodyLen;
