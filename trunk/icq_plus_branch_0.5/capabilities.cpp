...
const capstr capMimMobile        = {'M', 'i', 'r', 'a', 'n', 'd', 'a', 'M', 'o', 'b', 'i', 'l', 'e', 0, 0, 0}; //port by Nullbie
...
const capstr capXtrazVideo       = {0x17, 0x8C, 0x2D, 0x9B, 0xDA, 0xA5, 0x45, 0xBB, 0x8D, 0xDB, 0xF3, 0xBD, 0xBD, 0x53, 0xA1, 0x0A}; // TODO: dummy of capIcqLite
...
const capstr capOscarChat        = {0x74, 0x8F, 0x24, 0x20, 0x62, 0x87, 0x11, 0xD1, 0x82, 0x22, 0x44, 0x45, 0x53, 0x54, 0x00, 0x00}; // TODO: dummy of capAimChat
...
void CIcqProto::AddCapabilitiesFromBuffer(HANDLE hContact, BYTE* pbyBuffer, int nLength)
{
	...
	for (iCapability = 0; (iCapability + BINARY_CAP_SIZE) <= nLength; iCapability += BINARY_CAP_SIZE)
	{
		...
		if (!knownCap)
		{
			char szCap[64];
			for (int i = 0; i < 16; i++)
				sprintf( szCap + i * 3, " %02X", ( BYTE )( ( BYTE* )( pbyBuffer + iCapability )[i] ) );
			Netlib_Logf( m_hServerNetlibUser, "Found unknown CAP%s", szCap );
		}
	}
...
int CIcqProto::IcqAddCapability(WPARAM wParam, LPARAM lParam)
{
...
int CIcqProto::IcqCheckCapability(WPARAM wParam, LPARAM lParam)
{
	int res = 0;
	...
	if ( dbvariant.type == DBVT_BLOB )
		res = MatchCap( dbvariant.pbVal, dbvariant.cpbVal, ( capstr* )&icqCustomCap->caps, 0x10 ) ? 1 : 0;
...
void CIcqProto::AddCapabilitiesToBuffer( BYTE* packet, DWORD fdwContactCaps )
{
	int iCapability = 0;
	int nIndex;
	int nRecordSize;
...
const STDCAPINFO stdCapInfo[] = {
	{"Server Relay", 0, capSrvRelay, 16},
	{"NewCaps support", 0, capNewCap, 16},
	{"AIM support", 0, capAimIcq, 16},
	{"UTF-8 Messages", 0, capUTF, 16},
	{"Typing Notifications", 0, capTyping, 16},
	{"ICQ Xtraz", 0, capXtraz, 16},
	{"File Transfers (OscarFile)", 0, capOscarFile, 16},
...
char* CIcqProto::GetCapabilityName(BYTE *cap, ICQ_CAPINFO *info)
{
	...
	if (info)
	{
		...
		*ptr++ = '{';
		for (i = 0; i < 0x10; ++i)
		{
			sprintf(ptr, "0x%.2x%s", cap[i], ((i==0x0F)?"":", "));
			ptr += 4;
			ptr += ((i==0x0F)?0:2);
		}
		*ptr++ = '}';
