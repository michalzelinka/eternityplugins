...
// -----------------------------------------------------------------------------
//
// File name      : $URL: https://miranda.svn.sourceforge.net/svnroot/miranda/trunk/miranda/protocols/IcqOscarJ/icq_server.cpp $
// Revision       : $Revision: 7952 $
// Last change on : $Date: 2008-06-29 16:05:16 +0200 (Sun, 29 Jun 2008) $
...
void __cdecl CIcqProto::ServerThread(serverthread_start_info* infoParam)
{
  ...
  SAFE_FREE((void**)&infoParam);
	...
	if (hServerConn == NULL)
	{
		...
		ToggleAdvancedFeaturesItems( FALSE );

		if (m_bServerAutoChange)
			TryNextServer();
		else
	...
	ToggleAdvancedFeaturesItems( TRUE );
...
void CIcqProto::icq_serverDisconnect(BOOL bBlock)
{
	...
	if (hServerConn)
	{
		...
		// TODO: Why, when the function below does the same?
...
void CIcqProto::icq_login(const char* szPassword)
{
	...
	DBGetContactSettingTString(NULL, m_szModuleName, "OscarServer", &dbv);
	if (!dbv.ptszVal)
		stsi->nloc.szHost = null_strdup(DEFAULT_SERVER_HOST);
	else
#ifdef _UNICODE
	{
		WideCharToMultiByte(CP_ACP, 0, dbv.ptszVal, -1, szServer, sizeof(szServer), NULL, NULL);
		stsi->nloc.szHost = null_strdup(szServer);
	}
#else
		stsi->nloc.szHost = null_strdup(dbv.ptszVal);
#endif
...
DWORD __cdecl CIcqProto::icq_TryNextServerThread(LPVOID lp)
{
	WORD CurrServ, SrvCount, Port;
	char DBModule[64], buf[1024], buf1[64], buf2[64];

	m_bTryNextWorking = TRUE;

	WaitForSingleObject( serverThreadHandle, INFINITE );

	mir_snprintf( DBModule, 64, "%sSrvs", m_szModuleName );

	SrvCount = DBGetContactSettingWord( NULL, DBModule, "SrvCount", 0 );
	if ( SrvCount == 0 )
	{
		m_bTryNextWorking = FALSE;
		return FALSE;
	}

	CurrServ = DBGetContactSettingWord( NULL, DBModule, "CurrServ", 0 );

	if ( CurrServ >= SrvCount )
	{
		DBWriteContactSettingWord( NULL, DBModule, "CurrServ", 0 );
		icq_LogMessage( LOG_FATAL, "Server Autochange: Reached the end of servers' list, shutting thread down" );
		m_bTryNextWorking = FALSE;
		return FALSE;
	}
	...
	DBWriteContactSettingWord( NULL, DBModule, "CurrServ", CurrServ );
	...
	if( !DBGetContactSettingByte( NULL, DBModule, buf, 1 ) )
	{
		m_bTryNextWorking = FALSE;
		TryNextServer( );
		return FALSE;
	...
	Port = DBGetContactSettingWord( NULL, DBModule, buf2, DEFAULT_SERVER_PORT );	

	setSettingWord( NULL, "OscarPort", Port );
	{
		DBVARIANT dbv = { DBVT_TCHAR };
		DBGetContactSettingTString( NULL, DBModule, buf1, &dbv );
		DBWriteContactSettingTString( NULL, m_szModuleName, "OscarServer", dbv.ptszVal );
	}

	icq_LogMessage( LOG_NOTE, "Server Autochange: Connection problems with current server, switching to next" );

	SetStatus( ( WPARAM )m_iIcqNewStatus );
...
void CIcqProto::TryNextServer()
{
	if (m_bTryNextWorking)
		return;

	ForkThread((IcqThreadFunc)&CIcqProto::icq_TryNextServerThread, NULL);
}
