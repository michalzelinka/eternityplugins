...
{ // TODO: Jabber-like menu management
	CLISTMENUITEM mi = { 0 };
	...
	mi.ptszName = TranslateT(LPGEN("Advanced Features"));
	...
	mi.ptszName = TranslateT("Default, corresponding to status");
	...
	mi.ptszName = TranslateT("Allow all users to see you");
	mi.pszService = svcFnc;
	...
	mi.ptszName = TranslateT("Block all users from seeing you");
	mi.pszService = svcFnc;
	...
	mi.ptszName = TranslateT("Allow only users in the Visible list to see you");
	mi.pszService = svcFnc;
	...
	mi.ptszName = TranslateT("Block only users in the Invisible list from seeing you");
	mi.pszService = svcFnc;
	...
	mi.ptszName = TranslateT("Allow only users in the Contact list to see you");
	mi.pszService = svcFnc;
	...
	mi.ptszName = TranslateT("Allow only users in the Contact list to see you, except Invisible list users");
	mi.pszService = svcFnc;
	...
	mi.ptszName = TranslateT("Send global authorization request to all users who haven't authorized you yet");
	mi.pszService = svcFnc;
	...
	mi.ptszName = TranslateT("Active Status Discovery");
	mi.pszService = svcFnc;
	...
	mi.ptszName = TranslateT("Web Aware");
	...
	mi.ptszName = TranslateT("Manage server's list...");
	mi.pszService = svcFnc;
	...
	{
		null_snprintf(svcFnc, sizeof(svcFnc), "%s/%s%d", m_szModuleName, MS_ISEE_SCAN_USER_STATUS, i);
		CreateProtoServiceParam(svcFnc + len, &CIcqProto::menuAdvFeatures, i);

		CLISTMENUITEM smi = { 0 };
		smi.cbSize = sizeof(smi);
		smi.ptszPopupName = this->m_tszUserName;
		smi.pszService = svcFnc;
		smi.position = 1003;
		smi.flags = CMIF_TCHAR | CMIF_ICONFROMICOLIB | CMIF_HIDDEN;

		smi.icolibItem = hStaticIcons[ISI_SCAN_STOP]->Handle();
		smi.ptszName = _T("&Users Status Scan");

		m_hMenuUsersStatusScan = ( HANDLE )CallService( MS_CLIST_ADDSTATUSMENUITEM, 0, ( LPARAM )&smi );
	}
