...
// -----------------------------------------------------------------------------
//
// File name      : $URL: https://miranda.svn.sourceforge.net/svnroot/miranda/trunk/miranda/protocols/IcqOscarJ/fam_02location.cpp $
...
void CIcqProto::handleLocationUserInfoReply(BYTE* buf, WORD wLen, DWORD dwCookie)
{
	...
	setSettingWord(hContact, "AOLWarningLevel", wWarningLevel); // maybe we'll display it somewhere later
	...
	case CKT_CHECKSPAMBOT:
        ...
        if (wLen)
        {
				  ...
				  if (szClient == cliSpamBot)
				  {
					...
					if (status && getSettingWord(hContact, "Status", 0) == ID_STATUS_OFFLINE)
					{
