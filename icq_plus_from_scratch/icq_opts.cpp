...
static BOOL CALLBACK DlgProcIcqFeaturesOpts(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SetWindowLong( hwndDlg, GWL_USERDATA, lParam );
		{
			...
			LoadDBCheckState(ppro, hwndDlg, IDC_XSTNONSTD, "NonStandartXstatus", 0);
	...
	case WM_COMMAND:
		...
		case IDC_UTFENABLE:
			...
			break;
		case IDC_DCENABLE:
			state = IsDlgButtonChecked(hwndDlg, IDC_DCENABLE);
			icq_EnableMultipleControls(hwndDlg, icqDCMsgControls, SIZEOF(icqDCMsgControls), state);
			if (state)
				if (!IsDlgButtonChecked(hwndDlg, IDC_DCICON))
					icq_EnableMultipleControls(hwndDlg, icqDCIconControls, SIZEOF(icqDCIconControls), FALSE);
			break;
		case IDC_XSTATUSENABLE:
			state = IsDlgButtonChecked(hwndDlg, IDC_XSTATUSENABLE);
			icq_EnableMultipleControls(hwndDlg, icqXStatusControls, SIZEOF(icqXStatusControls), state);
			if (state)
			{
				if (!IsDlgButtonChecked(hwndDlg, IDC_XSTATUSICON))
					icq_EnableMultipleControls(hwndDlg, icqXStatusIconControls, SIZEOF(icqXStatusIconControls), FALSE);
				if (!IsDlgButtonChecked(hwndDlg, IDC_XSTUPDATE))
					EnableDlgItem(hwndDlg, IDC_XSTUPDATERATE, FALSE);
			}
			break;
		case IDC_DCICON:
			icq_EnableMultipleControls(hwndDlg, icqDCIconControls, SIZEOF(icqDCIconControls), IsDlgButtonChecked(hwndDlg, IDC_DCICON));
			break;
		case IDC_RTF:
		...
		case IDC_XSTATUSICON:
			icq_EnableMultipleControls(hwndDlg, icqXStatusIconControls, SIZEOF(icqXStatusIconControls), IsDlgButtonChecked(hwndDlg, IDC_XSTATUSICON));
			break;
		case IDC_XSTUPDATE:
	...
	case WM_NOTIFY:
		...
		case PSN_APPLY:
			...
			StoreDBCheckState(ppro, hwndDlg, IDC_XSTNONSTD, "NonStandartXstatus");
...
static BOOL CALLBACK DlgProcIcqFeaturesAdvOpts(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) 
{
	CIcqProto* ppro = (CIcqProto*)GetWindowLong(hwndDlg, GWL_USERDATA);
...
static BOOL CALLBACK DlgProcIcqEventsLogOpts(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	CIcqProto* ppro = (CIcqProto*)GetWindowLong(hwndDlg, GWL_USERDATA);
