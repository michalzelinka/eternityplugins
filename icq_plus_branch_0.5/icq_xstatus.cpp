...
// -----------------------------------------------------------------------------
//
// File name      : $URL: https://miranda.svn.sourceforge.net/svnroot/miranda/trunk/miranda/protocols/IcqOscarJ/icq_xstatus.cpp $
...
const capstr capXStatus[XSTATUS_COUNT] = {
	...
	{0xd6, 0x68, 0x7f, 0x4f, 0x3d, 0xc3, 0x4b, 0xdb, 0x8a, 0x8c, 0x4c, 0x1a, 0x57, 0x27, 0x63, 0xcd}};
...
void CIcqProto::InitXStatusItems(BOOL bAllowStatus)
{
	...
	for(i = 0; i <= XSTATUS_COUNT; i++) 
	{
		...
		if ( i % 19 == 0 ) mi.position += 1000020000; // divide into 2 columns
		mi.position++;
		...
		//mi.icolibItem = i ? hXStatusIconsHandle[i-1] : LoadSkinnedIconHandle( SKINICON_OTHER_SMALLDOT );
		// TODO: set Small Dot for None Custom Status selected
...
{ // TODO: Fork to show this one for older clients and Basic status message dialog for ICQ 6 clients
	InitXStatusData init;
	init.ppro = this;
