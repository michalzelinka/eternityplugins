  ...
  m_pendingAvatarsStart( 1 )
{
	...
	CreateResidentSetting("AdvStatus");
	...
	// Plus -- TODO: I think this should be done differently
	HookProtoEvent(ME_CLIST_EXTRA_LIST_REBUILD, &CIcqProto::CListMW_ExtraIconsRebuild_DC);
	HookProtoEvent(ME_CLIST_EXTRA_IMAGE_APPLY, &CIcqProto::CListMW_ExtraIconsApply_DC);
	...
	{ // additional sounds
		SkinAddNewSoundEx( "AuthRequest", m_szModuleName, Translate( "Authorization Request" ) );
		SkinAddNewSoundEx( "AuthDenied", m_szModuleName, Translate( "Authorization Denied" ) );
		SkinAddNewSoundEx( "AuthGranted", m_szModuleName, Translate( "Authorization Granted" ) );
		SkinAddNewSoundEx( "YouWereAdded", m_szModuleName, Translate( "You Were Added" ) );
		SkinAddNewSoundEx( "ContactRemovedSelf", m_szModuleName, Translate( "Contact Removed Self" ) );
		SkinAddNewSoundEx( "StatusScanComplete", m_szModuleName, Translate( "Status Scan Complete" ) );
	}

	{ // dialog handlers
		m_hRegDialog = NULL;
		m_hRegDialogCaller = NULL;
		m_hCapsList = NULL;
		m_hCapsListCaller = NULL;
		m_hASDDialogCaller = NULL;
		m_hSrvsList = NULL;
		m_hSrvsListCaller = NULL;
	}
	...
	//
	getSettingString(NULL, "HttpUserAgent", &dbv);
	if (!dbv.pszVal)
		lstrcpyA(szHttpAgent, DEFAULT_HTTP_AGENT);
	else
		lstrcpyA(szHttpAgent, dbv.pszVal);
	nlu.szHttpGatewayUserAgent = szHttpAgent;
	//
...
CIcqProto::~CIcqProto()
{
	...
	// TODO: possibly cleaning of handles, hooks, events, etc? :)
...
int CIcqProto::OnModulesLoaded( WPARAM wParam, LPARAM lParam )
{
	...
	char pszCapsName[MAX_PATH+4];
	char pszSrvsName[MAX_PATH+4];
	char* modules[7] = {0,0,0,0,0,0,0};
	...
	strcpy(pszCapsName, m_szModuleName);
	strcat(pszCapsName, "Caps");
	strcpy(pszSrvsName, m_szModuleName);
	strcat(pszSrvsName, "Srvs");
	modules[0] = m_szModuleName;
	modules[1] = pszP2PName;
	modules[2] = pszGroupsName;
	modules[3] = pszSrvGroupsName;
	modules[4] = pszCapsName;
	modules[5] = pszSrvsName;
	CallService("DBEditorpp/RegisterModule",(WPARAM)modules,(LPARAM)6);
...
int __cdecl CIcqProto::SendContacts( HANDLE hContact, int flags, int nContacts, HANDLE* hContactsList )
{
	if (hContact && hContactsList)
	{
		...
		else
		{
			...
			{
				dwCookie = ReportGenericSendError(hContact, ACKTYPE_CONTACTS, "The receiver does not support receiving of contacts.");
			}
		}
		return dwCookie;
	}
...
HANDLE __cdecl CIcqProto::GetAwayMsg( HANDLE hContact )
{
  ...
  if (!icqOnline())
	...
	if (dwUin)
	{
		...
		case ID_STATUS_ONLINE:
			{
				...
				DBGetContactSettingTString( hContact, m_szModuleName, "MirVer", &dbv );
		...
		{
			if (m_bDCMsgEnabled && IsDirectConnectionOpen(hContact, DIRECTCONN_STANDARD, 0) &&
			    !InvisibleForContact(0, hContact))
			{
				int iRes = icq_sendGetAwayMsgDirect(hContact, wMessageType);
				if (iRes) return (HANDLE)iRes; // we succeded, return
			}
			else if (m_bIncognitoRequest || m_bIncognitoGlobal)
			{
				m_bIncognitoRequest = FALSE; // TODO: FIX THIS RETURN/FUNCTION?
			...
			else if (CheckContactCapabilities(hContact, CAPF_STATUSMSG_EXT) && !InvisibleForContact(0, hContact))
				return (HANDLE)icq_sendGetAwayMsgServExt(hContact, dwUin, wMessageType,
				(WORD)(getSettingWord(hContact, "Version", 0)==9?9:ICQ_VERSION)); // Success
			else if (!InvisibleForContact(0, hContact))
