...
// -----------------------------------------------------------------------------
//
// File name      : $URL: https://miranda.svn.sourceforge.net/svnroot/miranda/trunk/miranda/protocols/IcqOscarJ/fam_13servclist.cpp $
...
void CIcqProto::handleServerCListItemDelete(const char *szRecordName, WORD wGroupId, WORD wItemId, WORD wItemType, oscar_tlv_chain *pItemData)
{
	...
	{ // a contact was removed from our list
		...
		{
			DWORD dwUin = getSettingDword(hContact, "UIN", 0);
			...
			}
			{
				...
				chk.dwUin = dwUin;
				chk.bShowPopup = chk.bHistoryEvent = chk.bLogToFile = TRUE;
				chk.iICQEventType = ICQEVENTTYPE_SELFREMOVE;
				chk.iPopupEvent = POPTYPE_SELFREMOVE;
				chk.pszMsg = "Removed himself from your server-list";
...
void CIcqProto::handleRecvAuthRequest(unsigned char *buf, WORD wLen)
{
	...
	if (DBGetContactSettingByte(hcontact, "CList", "NotOnList", 1))
	{
		...
		chk.iPopupEvent = POPTYPE_AUTH;
		chk.pszMsg = "Sent you 'You were added' notice";
		chk.iICQEventType = ICQEVENTTYPE_YOUWEREADDED;
		chk.iDBEventFlag = DBEF_READ;
		CheckContact(chk);
	}
	...
	*(char *)pCurBlob = 0;
	...
	SkinPlaySound("AuthRequest"); // Added by BM
	{
		...
		chk.bShowPopup = chk.bHistoryEvent = chk.bLogToFile = TRUE;
		chk.iPopupEvent = POPTYPE_AUTH;
		chk.pszMsg = "Authorization requested";
		chk.iICQEventType = ICQEVENTTYPE_AUTHREQUESTED;
		chk.iDBEventFlag = DBEF_READ;
		CheckContact(chk);
	}
	return;
...
void CIcqProto::handleRecvAdded(unsigned char *buf, WORD wLen)
{
	...
	*(char *)pCurBlob = 0;
	...
	if (m_bLogAuthHistory)
		HistoryLog(hContact, dwUin, "You were added to user's contact list", 0, DBEF_READ);
...
void CIcqProto::handleRecvAuthResponse(unsigned char *buf, WORD wLen)
{
	...
	case 0:
		...
		if (NotOnList)
		{
			...
			chk.hContact = hContact;
			chk.dwUin = dwUin;
			chk.bShowPopup = chk.bHistoryEvent = chk.bLogToFile = chk.bNotOnList = TRUE;
			chk.iPopupEvent = POPTYPE_VISIBILITY;
			chk.iDBEventFlag = DBEF_READ;
			chk.iICQEventType = ICQEVENTTYPE_CHECKSTATUS;
			chk.pszMsg = "Probably detected your invisibility (auth denied)";
			CheckContact(chk);
		}
		else
		{
			if (m_bLogAuthHistory)
				HistoryLog(hContact,dwUin, "has denied your authorization request", ICQEVENTTYPE_AUTHDENIED, DBEF_READ);
			else
				HistoryLog(0, 0, "has denied your authorization request", ICQEVENTTYPE_AUTHDENIED, 0);
			LogToFile(hContact, dwUin, 0, ICQEVENTTYPE_AUTHDENIED);
		}
	...
	case 1:
		...
		if (NotOnList)
		{
			...
			chk.dwUin = dwUin;
			chk.hContact = hContact;
			chk.bShowPopup = chk.bLogToFile = chk.bHistoryEvent = chk.bNotOnList = TRUE;
			chk.iPopupEvent = POPTYPE_VISIBILITY;
			chk.iDBEventFlag = DBEF_READ;
			chk.iICQEventType = ICQEVENTTYPE_CHECKSTATUS;
			chk.pszMsg = "Probably detected your invisibility (auth granted)";
			CheckContact(chk);
		}
		else
		{
			if (m_bLogAuthHistory)
				HistoryLog(hContact, dwUin, "Authorization request granted", ICQEVENTTYPE_AUTHGRANTED, DBEF_READ);
			else
				HistoryLog(0, 0, "Authorization request granted", ICQEVENTTYPE_AUTHGRANTED, 0);
			LogToFile(hContact, dwUin, 0, ICQEVENTTYPE_AUTHGRANTED);
		}
		// TODO: Add to system history as soon as new auth system is ready
		break;
	...
	default:
		...
		if (NotOnList)
		{
			...
			chk.iICQEventType = ICQEVENTTYPE_CHECKSTATUS;
			chk.iDBEventFlag = DBEF_READ;
			chk.pszMsg = "Unknown authorization request from a contact not in your list";
			CheckContact(chk);
		}
		break;
...
void CIcqProto::handleRecvRemoved(BYTE *buf, WORD wLen)
{
	if (wLen >= 10)
	{
		...
		if (unpackServerListItem(&buf, &wLen, szUid, &wGroupId, &wItemId, &wItemType, NULL))
		{
			...
			{ // a contact was removed from our list
				...
				icq_sendNewContact(0, szUid); // add to CS to see him
				{
					...
					chk.dwUin = dwUin;
					chk.iPSD = 29;
					chk.bHistoryEvent = chk.bLogToFile = TRUE;
					chk.iICQEventType = ICQEVENTTYPE_SELFREMOVE;
					chk.iDBEventFlag = DBEF_READ;
					chk.pszMsg = "User was removed from server list";
					CheckContact(chk);
				}
			}
		}
...
//   06 - Allow only users in the buddy list to see you, except Invisible users (pseudo)
//
void CIcqProto::updateServVisibilityCode(BYTE bCode)
{
	...
	if ((bCode > 0) && (bCode < 6))
	{
		...
		if (m_bVisibility == bCode) // if no change was made, not necescary to update that
			return;
