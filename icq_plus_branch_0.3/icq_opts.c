      ...
      case PSN_APPLY:
        {
          ...
          GetDlgItemText(hwndDlg, IDC_PASSWORD, str, 9);
          if (strlennull(str))
          {
...
static BOOL CALLBACK DlgProcIcqFeaturesOpts(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
	  {
		  ...
		  SendDlgItemMessage(hwndDlg, IDC_QIP_STATUS_ICON_POS, CB_SETCURSEL, (DBGetContactSettingWord(NULL,gpszICQProtoName,"qip_status_icon_pos", 9))-1, 0);//added
  ...
  case WM_COMMAND:
	  ...
	  case IDC_XSTUPDATE:
		  ...
		  OptDlgChanged(hwndDlg);
		  break;
	  case IDC_UTFALL: case IDC_RTF: case IDC_UTFCODEPAGE: case IDC_TEMPVISIBLE: case IDC_SLOWSEND:
	  case IDC_ONLYSERVERACKS: case IDC_DCPASSIVE: case IDC_XSTATUSAUTO: case IDC_QIP_STATUS_ICON_POS: 
	  case IDC_XSTATUS_ICON_POS: case IDC_DC_ICON_POS: case IDC_XSTATUS_SHOW: case IDC_QIPSTATUS_SHOW:
	  case IDC_XSTATUSRESET: case IDC_KILLUNKNOWN: case IDC_KILLSPAMBOTS:
	  case IDC_AIMENABLE: case IDC_REPLACEXTEXT: case IDC_FORCEXSTATUS:
	  case IDC_XSTNONSTD: case IDC_QIP_STATUS:
		  OptDlgChanged(hwndDlg);
		  break;
  ...
  case WM_NOTIFY:
	  ...
	  case PSN_APPLY:
		  ...
		  }
		  {
			  ...
			  DBWriteContactSettingWord(NULL, gpszICQProtoName, "qip_status_icon_pos", (WORD)i);
		  }
...
//		  icq_EnableMultipleControls(hwndDlg, icqIncognitoControls, sizeof(icqIncognitoControls)/sizeof(icqIncognitoControls[0]), IsDlgButtonChecked(hwndDlg, IDC_STEALTHRQST));
		  case IDC_ASDOPTIONS:
			  ...
			  OptDlgChanged(hwndDlg);
			  break;
		  case IDC_NOPSD_FOR_HIDDEN: case IDC_INCUSER: case IDC_INCGLOBAL: case IDC_INMAINMENU:
		  case IDC_INSTATUSMENU: case IDC_TZER: case IDC_SHOW_AUTH: case IDC_USRCHKPOS:
			  OptDlgChanged(hwndDlg);
			  break;
		  default:
			  return 0;
...
static BOOL CALLBACK DlgProcIcqEventLogOpts(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_BROWSE:
			...
			break;
		case IDC_LOG_REMOVE_FILE: case IDC_LOG_IGNORECHECK_FILE: case IDC_LOG_CHECKSTATUS_FILE:
		case IDC_LOG_CLIENTCHANGE_FILE: case IDC_LOG_AUTH_FILE: case IDC_LOG_REQUEST_FILE: case IDC_LOG_ASD_FILE:
		case IDC_LOG_READXSTATUS_FILE:
		case IDC_LOG_REMOVE_HISTORY: case IDC_LOG_IGNORECHECK_HISTORY: case IDC_LOG_CHECKSTATUS_HISTORY:
		case IDC_LOG_CLIENTCHANGE_HISTORY: case IDC_LOG_AUTH_HISTORY: case IDC_LOG_REQUEST_HISTORY:
		case IDC_LOG_ASD_HISTORY: case IDC_LOG_TO_HCONTACT_HISTORY: case IDC_LOG_READXSTATUS_HISTORY:
			OptDlgChanged(hwndDlg);
			break;
		case IDC_FILEPATH:
			{
				...
				if(strstr(UniGetContactSettingUtf(NULL, gpszICQProtoName, "EventsLog", "EventsLog.txt"),i))
					OptDlgChanged(hwndDlg);
			}
			break;
		default:
...
static BOOL CALLBACK DlgProcIcqContactsOpts(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_COMMAND:
    ...
    case IDC_DELETE_TMP_CONTACTS:
	  ...
	  OptDlgChanged(hwndDlg);
	  break;
	case IDC_ADDSERVER: case IDC_LOADFROMSERVER: case IDC_SAVETOSERVER: case IDC_ADDTEMP: case IDC_TMPSNDADDED:
	case IDC_TMPREQAUTH: case IDC_REMOVE_TEMP: case IDC_TMP_CONTACTS_GROUP: case IDC_STRICTAVATARCHECK:
	case IDC_AUTOLOADAVATARS: case IDC_BIGGERAVATARS:
		OptDlgChanged(hwndDlg);
		break;
	default:
		return 0;
