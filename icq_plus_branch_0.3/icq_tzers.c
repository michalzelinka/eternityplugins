	...
	switch (uMsg) {
		case WM_INITDIALOG:
...

			{
				...
				while(i<12)
				{
					SendDlgItemMessage(hWnd, IDC_TZER_COMBO, CB_ADDSTRING, 0, (LPARAM)(tZers[i].szTxt));
					i++;
				}
			}
			...
			SendDlgItemMessage(hWnd, IDC_TZER_ICON, STM_SETICON, (WPARAM)IconLibGetIcon("tzer0"), 0); 
		...
		case WM_COMMAND:
			if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == IDC_TZER_COMBO)
			{
				...
				SendDlgItemMessage(hWnd, IDC_TZER_ICON, STM_SETICON, (WPARAM)IconLibGetIcon(szTemp), 0); 
			}
...
static char* InitTzersIconLibrary(char* buf)
{
  ...
  if (!hIconsDLL)
     hIconsDLL = LoadTzersIconLibrary(path, "\\Icons");

  if (!hIconsDLL)
     hIconsDLL = LoadTzersIconLibrary(path, "\\Plugins");
...
void InitTzersIcons()
{
  ...
  null_snprintf(szSection, sizeof(szSection), ICQTranslateUtfStatic("Status Icons/%s/Tzers", str, MAX_PATH), ICQTranslateUtfStatic(gpszICQProtoName, prt, MAX_PATH));
