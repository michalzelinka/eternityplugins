...
void handleDirectMessage(directconnect* dc, PBYTE buf, WORD wLen)
{
      ...
      default: 
        {
          ...
          handleMessageTypes(dc->dwRemoteUin, time(NULL), 0, 0, wCookie, dc->wVersion, (int)bMsgType, (int)bMsgFlags, 0, (DWORD)wLen, wTextLen, buf, MTF_DIRECT, &pMsgAck);
          break;
        }
    }
  else if (wCommand == DIRECT_ACK)
  {
    ...
    { // this is status reply
      ...
      handleMessageTypes(dc->dwRemoteUin, time(NULL), 0, 0, wCookie, dc->wVersion, (int)bMsgType, (int)bMsgFlags, 2, (DWORD)wLen, wTextLen, buf, MTF_DIRECT, NULL);
    }
    else
    {
...
void handleDirectGreetingMessage(directconnect* dc, PBYTE buf, WORD wLen, WORD wCommand, WORD wCookie, BYTE bMsgType, BYTE bMsgFlags, WORD wStatus, WORD wFlags, char* pszText)
{
  ...
  else if (typeId && wCommand == DIRECT_MESSAGE)
  {
    ...
    handleMessageTypes(dc->dwRemoteUin, time(NULL), 0, 0, wCookie, dc->wVersion, typeId, 0, 0, dwLengthToEnd, (WORD)dwDataLength, buf, MTF_DIRECT, &pMsgAck);
  }
  ...
  { // especially for icq2003b
    ...
    handleMessageTypes(dc->dwRemoteUin, time(NULL), 0, 0, wCookie, dc->wVersion, (int)(qt + 0xE7), 3, 2, (DWORD)wLen, (WORD)dwDataLength, szMsg, MTF_DIRECT, NULL);
  }
