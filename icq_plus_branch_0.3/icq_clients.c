...
char* detectUserClient(HANDLE hContact, DWORD dwUin, WORD wUserClass, WORD wVersion, DWORD dwFT1, DWORD dwFT2, DWORD dwFT3, DWORD dwOnlineSince, BYTE bDirectFlag, DWORD dwDirectCookie, DWORD dwWebPort, BYTE* caps, WORD wLen, BYTE* bClientId, char* szClientBuf)
{
  ...
  { // capabilities based detection
    ...
    if (dwUin && caps)
    {
      ...
      switch (iver2) {
        case 0x58 : lstrcatA(szTmp, " eternity/PlusPlus++"); break; // detecting naturalized eternity/PlusPlus++ mod
        default : break;
      }
			lstrcatA(szTmp, " ");
      switch (iver0) {
      ...
      else if (szClient == NULL && !((capId = MatchCap(caps, wLen, &capIcqJs7, 4))||(capId = MatchCap(caps, wLen, &capIcqJSin, 4))||(capId = MatchCap(caps, wLen, &capIcqJp, 4))||(capId = MatchCap(caps, wLen, &capIcqJen, 4)))) // HERE ENDS THE SIGNATURE DETECTION, after this only feature default will be detected
      {
		...
		{ // try to determine lite versions
		  ...
		  {
			  if (MatchCap(caps, wLen, &capAimIcon, 0x10) && MatchCap(caps, wLen, &capAimContactSnd, 0x10) && MatchCap(caps, wLen, &capAimChat, 0x10) && MatchCap(caps, wLen, &capLiveVideo, 0x10) && MatchCap(caps, wLen, &capAimAudio, 0x10))
