...
// Last change on : $Date: 2007-08-10 11:01:07 +0300 (ÐŸÑ‚, 10 Ð°Ð²Ð³ 2007) $
...
void InitAvatars()
{
  ...
  { // do it only once
    ...
    { // check if it does make sense
	  ...
	  if (_mirandaVersion >= PLUGIN_MAKE_VERSION(0, 8, 0, 29))
	  {
		  char *tmpPath = Utils_ReplaceVars("%miranda_avatarcache%");
		  null_snprintf(szPath, MAX_PATH, "%s\\%s\\", tmpPath, gpszICQProtoName);
		  mir_free(tmpPath);
	  }
	  else
...
void GetAvatarFileName(int dwUin, char* szUid, char* pszDest, int cbLen)
{
  ...
  if (CallService(MS_FOLDERS_GET_PATH, (WPARAM)hAvatarsFolder, (LPARAM)&fgd))
  {
	  ...
	  if (_mirandaVersion >= PLUGIN_MAKE_VERSION(0, 8, 0, 29))
	  {
		  tmpPath = Utils_ReplaceVars("%miranda_avatarcache%");
		  lstrcpynA(pszDest, tmpPath, cbLen-1);
	  }
	  else
		  CallService(MS_DB_GETPROFILEPATH, cbLen, (LPARAM)pszDest);
	  tPathLen = null_snprintf(pszDest, cbLen, "%s\\%s\\", tmpPath, gpszICQProtoName);
	  mir_free(tmpPath);
  }
  else
  {
