          ...
          *szEndSend = '\0';
          ...
          if ((DWORD)atoi(szSender) == dwUin)
          {
            ...
            { // apply privacy rules
			  ...
			  if(bReadXStatusPopUp)
			  {
			  //ShowPopUpMsg(hContact, dwUin, NickFromHandleUtf(hContact), "Contact reading your xtraz message.", LOG_NOTE);
			  {
					CHECKCONTACT chk = {0};
					...
					chk.icqeventtype=ICQEVENTTYPE_CHECK_XSTATUS;
					chk.popuptype=POPTYPE_VIS;
					chk.msg="(xtraz message)";
					CheckContact(chk);
				}
			  }
