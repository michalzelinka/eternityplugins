...
void LoadServerIDs()
{
  ...
  {
      DBVARIANT dbv = {0}; 
 	  if (!DBGetContactSettingW(NULL, gpszICQProtoName, "SrvUnhandledIDList", &dbv)) 
 	  { 
 	...
 	if (pdwServerIDList) 
 	    for (i = 0; i<nServerIDListCount; i++) 
 	        ...
 	        if (pUnhandled) 
 				...
 				DBDeleteContactSetting(NULL, gpszICQProtoName, "SrvUnhandledIDList"); 
} 
